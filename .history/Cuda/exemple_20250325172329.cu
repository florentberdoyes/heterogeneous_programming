...
// Fonction Kernel qui permet d'effectuer la parallelisation du code exclusivement sur la partie "process image" 
...
void bilateral_filter(unsigned char *src, unsigned char *dst, int width, int height, int channels, int d, double sigma_color, double sigma_space) {
    ...
    // Cette partie du code remplace la partie "process image" 
    // Dans cette partie, on copie les différentes informations qui étaient 
    // nécessaire à cette partie pour les transmettre au GPU et vice versa 
